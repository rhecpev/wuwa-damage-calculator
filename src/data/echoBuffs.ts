...
export const echoAbilityBuffs: Record<string, EchoBuffTemplate[]> = {
  ...
  //
  // 109.44% 인멸 피해와 「수많은 기억 속 검의 소리」 41.04%(암흑 효과 부여 때마다 1자루씩,
  // 4자루)는 둘 다 에코가 스스로 때리는 공격이라 공격 데이터 쪽이다(echoAttackOverrides.ts).
  "6000218": [
    {
      label: "메인 슬롯 장착 시 인멸 피해 보너스",
      target: "damageBonus",
  ...
  // 봉정계유 (6000216)
  //   메인 슬롯 장착 시 치료 효과 보너스 10.00% — 피해식에는 안 들어가지만 스탯창에 찍힌다
  //   237.60% 응결 피해는 어빌리티 자체 배율이라 공격 데이터 쪽이다
  "6000216": [
    {
      label: "메인 슬롯 장착 시 치료 효과 보너스",
      target: "healingBonus",
      damageType: "All",
      value: 0.1,
      uptime: "passive", // 피해식에는 안 들어가고 스탯창에만 찍힌다
      scope: "self",
      condition: "메인 슬롯에 장착 시",
    },
  ],
  ...
  //
  // 「시야 붕괴」 2단 × 131.04%와 핵심 붕괴 추가타는 둘 다 공격 데이터 쪽이다
  ...
  // 금희 (60200401)
  //   메인 슬롯에 해당 에코 어빌리티를 장착 시 자신의 협동 공격이 입히는 피해가 40.00% 증가
  //
  // 헤카테(6000085)와 설명이 글자까지 같다 — 60200401~60200409는 쿨타임만 다르다.
  // 협동 공격은 damageType "Chain"으로 걸린다(에코 세트 「하늘의 합주곡」과 같은 자리다).
  // damageBonusType이 따로 적힌 협동 공격(연무의 뇌전의 쐐기 등 공명 스킬로 판정되는 것)에는
  // 걸리지 않는다.
  //
  // 미반영 — 어빌리티 자체의 피해(「명월의 시녀」 회전칼 45.59% 인멸)와
  //   패링 성공 시 존재 시간 리셋은 공격 데이터 · 판정 쪽이다.
