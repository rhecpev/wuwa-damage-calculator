...
const passiveBuffs: CharacterBuffTemplate[] = [
  ...
  // 이 줄은 맨 위 「경청하는 마음 · 광학 효과 피해 부스트 (고해)」와 같은 효과라 둘 다 켜면 두 번 걸렸다.
  // 위 줄 하나만 남기고 이 줄은 자리만 남긴다(retired) — 지우면 뒤 줄의 버프 열쇠가 당겨진다.
  {
    label: "묵념 · 광학 효과 피해 부스트 (중복 · 효과 없음 — 맨 위 고해 줄을 켠다)",
    target: "anomalyBoost",
    damageType: "SpectroFrazzle",
    value: 0,
    ...
    uptime: "active",
    scope: "party",
    condition: "맨 위 「경청하는 마음 · 광학 효과 피해 부스트 (고해)」와 같은 효과",
  },
  {
  ...
  },
  {
    ...
    exclusiveGroup: "phoebe-c2-state",
    resonanceChain: 2,
    condition: "고해 상태. 맨 위 고해 광학 효과 부스트에 더해진다 (사죄 쪽과 하나만 켠다)",
  },
  // 반주 「경청하는 마음」 사죄 상태 — 「피해 배율이 255% 상승된다」(곱연산 상승).
  // 담아 둔 사이클의 버프 열쇠가 배열 순번이라(character:<id>:<순번>), 반주 자리에
  // 끼우지 않고 맨 뒤에 붙였다 — 중간에 끼우면 뒤 줄의 체크가 밀린다.
  {
    label: "경청하는 마음 · 반주 배율 상승 (사죄)",
    target: "motionValue",
    damageType: "All",
    modifier: "amplify",
    attackIds: ["1003009_1"],
