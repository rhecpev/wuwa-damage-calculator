...
/**
 * 부옵션(무기 두 번째 옵션)은 원본 값을 그대로 쓴다.
 *
 * 예전에는 퍼센트 소수 둘째 자리부터 버려 36.45% → 36.4%로 썼다. 게임 화면이 한 자리까지만
 * 보여 주기 때문이었는데, 실측이 그 반대라고 말한다 —
 *   젠니 Lv.90 · 물결의 파동(공격력 36.45%) · 기초 1024 → 게임 공격력 2557
 *     36.4%로 깎으면 ⌊1024×1.484⌋ = 1519, 36.45% 그대로면 ⌊1024×1.4845⌋ = 1520 ← 이쪽이 맞는다
 * 되돌리려면 Math.floor(v * 1000) / 1000 로 두면 된다.
