...
export const ANOMALY_RATES: Record<AnomalyKind, number[]> = {
  ...
  //
  // 앞 두 칸은 실측으로 갈아 끼웠다. 수수 Lv.90 · 적 Lv.100 회절(저항 20%)에서
  //   1스택 게임 352 → 배율 ∈ (24.4477%, 24.5174%]  (원문 표 24.61%로는 354)
  //   2스택 게임 638 → 배율 ∈ (44.3681%, 44.4378%]  (원문 표 44.47%로는 639)
  // 두 구간을 한꺼번에 만족하는 깔끔한 값이 24.5 · 44.4다. 원문 표에 한 배수를 곱해서는
  // 두 점을 동시에 맞출 수 없다(1스택은 0.9934~0.9962배, 2스택은 0.9979~0.9995배를 요구).
  //
  // 3스택 이상은 아직 원문 표 그대로다 — 실측이 없다. 4스택까지는 어느 쪽으로 읽어도
  // 값이 같고, 5스택부터 갈린다(원문 표 1494 · 등차 19.9로 읽으면 1495).
  FrostChafe: [
    0.245, 0.444, 0.6433, 0.8418, 1.0404, 1.239, 1.4376, 1.6362, 1.8391, 2.0377,
