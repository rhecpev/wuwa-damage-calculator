...
export function calculateFinalStats(
   ...
   *
   * 스탯창 단계의 버림은 **두 번**이다 — 캐릭터 쪽 패널%(스킬 트리 · 무기)와 에코 옵션%를
   * 갈라 각각 버린 뒤 더한다.
   *
   * 2026-09-07에 한 번 버림으로 바꿨다가 되돌렸다(2026-09-24). 그때는 치사 공격력이
   *   ⌊937×1.24⌋ + ⌊937×0.8772⌋ = 1161 + 821 = 1982 (게임 1983)
   * 으로 1 모자라서 버림을 한 번으로 합쳤는데, 모자란 원인은 버림이 아니라 그 전날 공격력% 10.1의
   * 먹히는 값을 10.02로 내린 것이었다(예전 표시값 87.8%로는 822가 나와 맞았다).
   * 상리요 실측 두 건이 갈라 버림만 맞는다 —
   *   에코 전부   ⌊837×1.12⌋ + ⌊837×0.54⌋  = 937 + 451 = 1388 → +430 = 1818 (한 번이면 1819)
   *   사냥꾼 뺌   ⌊837×1.12⌋ + ⌊837×0.266⌋ = 937 + 222 = 1159 → +430 = 1589 (한 번이면 1590)
   * 10.1을 10.05로 올려(현령 9.972~10.071 · 치사 10.027 이상) 치사 · 현령도 함께 맞춘다.
   * 파수인 HP 49915는 HP% 메인 22.8을 22.7905로 두면 두 모양 모두에서 맞는다(echoStats.ts).
   */
  const resolve = (
    base: number,
    percent: number,
