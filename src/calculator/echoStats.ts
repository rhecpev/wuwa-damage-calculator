 ...
 * 그 표시값보다 **0.008%p 낮다** — 표시값이 반올림된 것이다. 스탯창 계산이 버림이라
 * 이 차이가 결과를 1~2 어긋나게 만든다.
 *
 ...
 *     0.008%p 낮게: ⌊10775×0.07892⌋+2280 = 850+2280 = 3130 ← 일치
 *   HP% 7.9+10.1 + 메인 서브 옵션 HP 2280 셋      → 게임 +8777
 *     표시값 그대로: ⌊10775×0.180⌋+6840 = 1939+6840 = 8779  (2 어긋남)
 *     0.008%p 낮게: ⌊10775×0.17984⌋+6840 = 1937+6840 = 8777 ← 일치
 *
 * 어긋나는 양이 부옵션 개수를 따라가므로 「버림 위치」 문제가 아니다. 그렇다고
 * 「옵션마다 1씩 깎는다」로 두면 앞서 맞춰 둔 공격력 실측(2564)이 2557로 깨진다.
 * 퍼센트값 자체가 낮다고 보는 쪽만 실측을 동시에 맞춘다
 * (공격력 쪽 에코 87.8% → 87.7x%, ⌊937×0.877x⌋ = 822로 그대로다).
 *
 * 깎는 폭이 0.01%p가 아니라 0.008%p인 것은 HP 실측 두 건이 더 붙어서 잡혔다.
 *   단근 Lv.90(기초 HP 9437) · HP% 11.6+8.6+8.6 · 깡 390+430+2280+2280 → 게임 +8095
 *     0.01%p:  ⌊9437×0.2877⌋+5380 = 2715+5380 = 8095   ← 일치
 *     0.008%p: ⌊9437×0.28776⌋+5380 = 2715+5380 = 8095  ← 일치
 *   현령 Lv.90(기초 HP 11025) · HP% 8.6+10.9 · 깡 510+2280+2280 → 게임 +7218
 *     0.01%p:  ⌊11025×0.1948⌋+5070 = 2147+5070 = 7217  (1 모자람)
 *     0.008%p: ⌊11025×0.19484⌋+5070 = 2148+5070 = 7218 ← 일치
 * 네 건을 동시에 맞추는 폭은 0.007~0.0085%p뿐이라 그 사이의 0.008%p로 둔다.
 *
