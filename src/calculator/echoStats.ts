 ...
 * 그래서 값마다 따로 잡는다.
 *
 * 아래 값은 실측 일곱 건이 허용하는 구간에서 여유가 가장 큰 점을 고른 것이다(전부 표시값
 * ±0.05%p 안). 구간 자체가 좁지 않아 값 하나하나가 확정된 것은 아니다 — 실측이 더 모이면
 * 다시 잡아야 한다. 맞춘 실측은 docs/피해-실측-대조.md 참고.
 *   수수 HP 25703 · 단근 HP 17532 · 현령 HP 18243 · 히유키 HP 16722
 *   현령 공격력 2434 · 히유키 공격력 2381 · 루실라 공격력 2187
...
const subPanelPercent = (shown: number): number =>
  SUB_PANEL_PERCENT[String(shown)] ?? shown - SUB_PERCENT_ADJUST;
