 ...
 *
 * **2026-09-17 기준 스탯 실측은 전부 맞는다**(HP · 공격력 · 방어력 스물한 칸).
 * 한때 이 자리에 「페비 공격력 2336 · 벨리나 공격력 1612 · 카르티시아 HP 50102가 어긋난다」고
 * 적혀 있었는데 셋 다 풀렸다 — 앞의 둘은 공격력% 부옵션을 표시값으로 되돌리면서, 카르티시아는
 * 아래 메인 옵션 표(MAIN_PANEL_PERCENT)가 들어오면서 맞았다. 앱에서 다시 확인한 값 —
 *   페비 공격력   ⌊912 × 1.24⌋ + ⌊912 × 0.84⌋ + 깡 440 = 2336
 *   벨리나 HP     ⌊14237 × 1.0⌋ + ⌊14237 × 0.1568⌋ + 깡 5350 = 21819
 *   카르티시아 HP ⌊14800 × 1.9623⌋ + ⌊14800 × 0.9264⌋ + 깡 7350 = 50102
 * 살아 있는 대조표는 늘 docs/피해-실측-대조.md의 「스탯 실측 대조」다 — 값을 손보면 그 표로 되짚는다.
 *
...
const SUB_PANEL_PERCENT: Record<string, Record<string, number>> = {
  ...
  atkPercent: {
    "7.1": 6.98,
    // 현령 공격력 2434가 9.972~10.071, 치사 공격력 2283이 10.027 이상을 요구한다(스탯창 갈라 버림 기준).
    "10.1": 10.05,
  },
...
const MAIN_PANEL_PERCENT: Record<string, Record<string, number>> = {
  hpPercent: {
    "33.0": 32.986,
    "30.0": 29.988,
    // 22.79면 스탯창을 갈라 버릴 때 파수인이 49914로 1 모자란다. 22.7905면 두 모양 모두 49915.
    "22.8": 22.7905,
  },
};
